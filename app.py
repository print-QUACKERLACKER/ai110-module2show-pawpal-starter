...
if st.button("Generate schedule"):
    if not st.session_state.tasks:
        ...
        tasks = [
            Task(
                title=t["title"],
                category=TaskType(t.get("category", "other")),
        ...
        plan = scheduler.build_plan(pet, tasks)

        st.success(f"Daily plan for {pet.name} ({pet.species})")
        if plan.entries:
            st.table(
                [
                    {
                        ...
                        "Priority": e.task.priority.name.lower(),
                    }
        ...
        st.code(scheduler.explain(plan), language="text")
